...
router.get('/transactions', authenticate, async (req, res) => {
  try {
    const transactions = await wallet.getTransactions(req.user.id);
...
router.post('/topup', authenticate, async (req, res) => {
  try {
    ...
    const newBalance = await wallet.addFunds(req.user.id, parseFloat(amount));
...
router.post('/charge', authenticate, async (req, res) => {
  try {
    ...
    const result = await wallet.deductFunds(req.user.id, parseFloat(amount));
